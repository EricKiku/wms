...
import { addOutApi, deleteOutApi } from "@/api/apis"
import OutKuListTable from "@/components/OutKuListTable.vue"
...
function handleSubmitIn() {
    // 入库编号，日期时间戳
    let outCode = new Date().getTime() + ''
    let uId = store.getUser.uId
    for (const product of products.value) {
        if (!outType.value || !outWarehouse.value || !uId) {
        ...
        addOutApi({
            ...
            outWarehouse: outWarehouse.value,
            outAddress: address.value,
            pId: product.pId,
            outAmount: product.amount,
            outDate: inTime.value,
            uId,
        }).then(res => {
            if (res.data.code === 200) {
                message('出库成功', 'success')
                outkulist.value.refresh()
...
function handleDeleteIn() {
    ElMessageBox.confirm(
        ...
        .then(() => {
            for (const product of outkulist.value.table.selectedProducts) {
                deleteOutApi(product.outId).then(res => {
                    if (res.data.code === 200) {
                        outkulist.value.refresh()
                        // 清空选中
                        outkulist.value.table.selected = []
...
function handleSearch() {
    outkulist.value.search(searchKey.value)
}
</script>
...
<template>
    <div class="modelIntroduct">
        <div class="modelTitle">
            <img src="/src/assets/icons/outku.png" alt="">
            <h2>出库管理</h2>
        </div>
        <p>对出库产品的记录管理，有展示出库记录，添加出库记录，搜索和删除的功能</p>
    </div>
    ...
    <el-dialog v-model="addDialogShow" width="auto" :show-close="false" align-center>
        ...
        <div class="dia-container">
            <div :style="{ color }" class="title">出库基本信息</div>
            <div class="dia-row dia-baseinfo">
                <div class="col">
                    <div class="label"> <span class="require">*</span> 出库编号</div>
                    <el-input disabled placeholder="自动生成" />
                </div>
                <div class="col">
                    <div class="label"> <span class="require">*</span> 出库类型</div>
                    <el-select v-model="outType" class="m-2" placeholder="入库类型" style="width: 240px">
                        <el-option label="销售出库" value="销售出库" />
                        <el-option label="借出出库" value="借出出库" />
                        <el-option label="调拨出库" value="调拨出库" />
                        <el-option label="其他" value="其他" />
                    </el-select>
                </div>
                <div class="col">
                    <div class="label"> <span class="require">*</span> 出库仓库</div>
                    <el-select :loading="getWarehouseLoading" loading-text="加载中..." @visible-change="handleWarehouse"
                        v-model="outWarehouse" class="m-2" placeholder="Select" style="width: 240px">
            ...
            <div class="dia-row dia-baseinfo">
                <div class="col">
                    <div class="label"> <span class="require">*</span> 收货地址</div>
                    <el-input v-model="address" placeholder="收货地址" />
                </div>
            </div>
            <div :style="{ color }" class="title">出库产品信息</div>
            <div class="dia-row dia-baseinfo">
                <ProductTable v-model="products" />
            </div>
            <div :style="{ color }" class="title">出库确认</div>
            <div class="dia-row dia-baseinfo">
                <div class="col">
                    <div class="label"> <span class="require">*</span> 出库时间</div>
                    <el-date-picker v-model="inTime" type="date" placeholder="Pick a Date" format="YYYY/MM/DD"
                        value-format="YYYY-MM-DD" />
                </div>
                <div class="col">
                    <div class="label"> <span class="require">*</span> 出库人</div>
